...
class EthernetFrame:
    length = 14 # grab the first 14 bytes of this element
    def __init__(self, data):
        unpacked_data = unpack('!6s6sH', data[0:self.length]) # slice up first 14
...
class IPHeader:
    length = 20 #this is length of IP header, must match format code size
    def __init__(self, data):
        unpacked_data = unpack('!BBHHHBBH4s4s', data[0: self.length]) # format code=20
        ...
        self.leftover_data = data[self.length:] #after stripping ethframe
